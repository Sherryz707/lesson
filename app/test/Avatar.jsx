...
export default function Avatar({ ...props }) {
  ...
  const group = useRef();
  const { nodes, materials, scene, animations } = useGLTF("/models/girl-4.glb");
  ...
  const animationsArr =
    lessons.length > 0 ? lessons[currentLessonIndex].animations : null;
  let jsonFile;
  ...
  if (replayAnim.length > 0) {
    ...
    animationsArr &&
    animationsArr.length > 0 &&
    animIndex < animationsArr.length
  ) {
    jsonFile = useLoader(
      THREE.FileLoader,
  ...
  useEffect(() => {
    ...
    if (
      ...
      state.status == "active" &&
      animIndex >= animationsArr.length - 1
    ) {
      setFacialExpression("funnyFace");
      console.log(
        "FADING OUT OF IN AVATAR",
        animationsArr.slice(-1)[0].animation
      );
