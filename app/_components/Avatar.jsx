...
import { corresponding, facialExpressions, lerpMorphTarget } from "./ModelUtils"
import { randInt } from "three/src/math/MathUtils";

export default function Avatar({...props }) {
  const { state,animFinishShowModal,playIndivAnim,playIndivAnimFin,SetAnimToFade,EmptyFadeAnim } = useAppContext();
  const {lessons, currentLessonIndex,playIndivAnim:replayAnim,animToFade,animFadeStatus}=state
  console.log("lessons",state,lessons,currentLessonIndex,lessons[currentLessonIndex])
  const group = useRef();
  const { nodes, materials,scene } = useGLTF("/models/girl.glb");
  ...
  // useEffect(() => {
  //   console.log("FADE STATUS CHANGES BOIZ",animToFade)
  //   if (animToFade) {
  //             actions[animToFade].fadeOut(0.5)
  ...
  // },[animFadeStatus])
    ...
    () => {
        ...
        if (replayAnim.length > 0) {
          // if (animToFade) {
          //   console.log("animation to fade in replay useeffect",animToFade)
          //   actions[animToFade.animation].fadeOut(0.5);
          // }
        console.log(replayAnim, replayAnim ? "meow" : "NOOO")
        ...
        if (currLesson.animation) {
          console.log("curr lesson to acces in actions replay", currLesson.animation)
    ...
    useEffect(() => {
        ...
        const handleAudioEnd = () => {
            isAudioEnded.current = true;
            if(animIndex!=animationsArr.length-1){
                isAnimEnded.current?currentAction.fadeOut(0.5):null
            }
            if (isAnimEnded.current) {
                isAnimEnded.current = false;
                isAudioEnded.current = false;
        ...
        if (state.status == "active" && lessons.length > 0 && animIndex < animationsArr.length) {
            ...
            if (currLesson.animation) {
                console.log("curr lesson to acces in actions",currLesson.animation)
                currentAction = actions[currLesson.animation];
                currentAction.reset().fadeIn(0.2).play();
                currentAction.loop = THREE.LoopOnce;
                currentAction.clampWhenFinished = true;
            ...
            console.log("anim index",animIndex,animationsArr.length - 1)
